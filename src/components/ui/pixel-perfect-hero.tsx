...
/** Inline GitHub mark — lucide-react dropped the icon in newer versions */
...
/* ---------------------------------------------------------------------------
 * PIXEL CANVAS — staggered physics ripple engine
 * -------------------------------------------------------------------------- */
type Pixel = {
  x: number;
  y: number;
...
export function PixelHero({
  ...
  useEffect(() => {
    ...
    // Fixed black & gray pixel palette — 5 shades from near-invisible to mid-gray
    setThemeColors([
      "rgba(255,255,255,0.04)",  // almost invisible — fills the dark field
      "rgba(255,255,255,0.07)",
      "rgba(255,255,255,0.11)",
      "rgba(255,255,255,0.16)",
      "rgba(255,255,255,0.24)",  // brightest dot — sparse highlights
    ]);
    ...
    <div className="relative w-full min-h-[100dvh] bg-background flex flex-col justify-between md:justify-center md:gap-6 py-8 md:py-0 px-2 sm:px-6 overflow-hidden select-none isolate">
      ...
      >
        <button
          ...
          className="relative inline-flex h-10 md:h-12 items-center justify-center gap-1.5 md:gap-2 rounded-xl bg-gradient-to-b from-white/95 to-white/85 px-4 md:px-8 text-xs md:text-sm font-semibold text-black shadow-[inset_0_1px_1px_rgba(255,255,255,0.9),0_2px_4px_rgba(0,0,0,0.4),0_12px_24px_rgba(0,0,0,0.3)] ring-1 ring-white/20 transition-transform duration-200 hover:scale-[1.02] active:scale-[0.98] cursor-pointer"
        >
          <span className="inline md:hidden">{primaryCtaMobile}</span>
          <span className="hidden md:inline">{primaryCta}</span>
          <ArrowRight className="w-3.5 h-3.5 md:w-4 md:h-4" aria-hidden="true" />
        </button>
        {githubUrl.startsWith("/") ? (
          <Link
            href={githubUrl}
            onClick={onSecondaryClick}
            className="relative inline-flex h-10 md:h-12 items-center justify-center gap-1.5 md:gap-2 rounded-xl bg-gradient-to-b from-white/10 to-white/5 px-4 md:px-8 text-xs md:text-sm font-semibold text-white/80 shadow-[inset_0_1px_1px_rgba(255,255,255,0.08),0_2px_4px_rgba(0,0,0,0.3),0_12px_24px_rgba(0,0,0,0.2)] ring-1 ring-white/10 backdrop-blur-md transition-transform duration-200 hover:scale-[1.02] active:scale-[0.98] cursor-pointer"
          >
